...
class WhisperTranscriptionFlow(FlowSpec):
    ...
    def do_transcription(self):
        ...
        transcribe_languages = self.determineTranscriptionLanguage(self.theLocales[0])
        for index, language in enumerate(transcribe_languages):
            print ('the languages in transcribe_language list are: ', index, language)
    
            # language should be of type str
            print(f"Type: {type(language)}")
            print(f"Value: {language}")
            print('using transcription language: ', language, ' for locale: ', theLocale)
            ...
            for index, row in self.df.iterrows():
                ...
                for index, model in enumerate(self.theModels):
                    ...
                    transcription = whisper_model.transcribe(audio=audio, language=language)
                    print('transcription: ', transcription['text'])
                    self.transcription_df.loc[index, column_name] = transcription['text']
            ...
            self.transcription_df.to_csv('whisper_transcriptions' + '_' + theLocale + '_' + language + '.csv', sep='\t', index=True)
